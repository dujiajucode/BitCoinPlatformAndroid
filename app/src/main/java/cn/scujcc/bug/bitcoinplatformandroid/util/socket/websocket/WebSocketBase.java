...
public abstract class WebSocketBase {
    ...
    private static final String TAG = "WebSocketBase";
    ...
    public void start() {
        if (url == null) {
            Log.e(TAG, "WebSocketClient start error  url can not be null");
            return;
        }
        if (service == null) {
            Log.e(TAG, "WebSocketClient start error  WebSocketService can not be null");
            return;
        }
     ...
     * @param contractType
     */
    public void cancelFutureOrder(String apiKey, String secretKey, String symbol, long orderId, String contractType) {
        Log.e(TAG, "apiKey=" + apiKey + ", secretKey=" + secretKey + ", symbol=" + symbol + ", orderId=" + orderId
        ...
        StringBuilder tradeStr = new StringBuilder(
                "{'event': 'addChannel','channel': 'ok_futuresusd_cancel_order','parameters': ").append(params)
                .append("}");
    ...
    public void futureRealtrades(String apiKey, String secretKey) {
        Log.e(TAG, "apiKey=" + apiKey + ", secretKey=" + secretKey);
        ...
        StringBuilder tradeStr = new StringBuilder(
                "{'event':'addChannel','channel':'ok_sub_futureusd_trades','parameters':{'api_key':'").append(apiKey)
                .append("','sign':'").append(signStr).append("'},'binary':'true'}");
    ...
    public void futureTrade(String apiKey, String secretKey, String symbol, String contractType, double price,
        ...
        // 交易json
        StringBuilder tradeStr = new StringBuilder(
                "{'event': 'addChannel','channel':'ok_futuresusd_trade','parameters':").append(params).append("}");
        Log.e(TAG, tradeStr.toString());
        this.sendMessage(tradeStr.toString());
    ...
    public void getUserInfo(String apiKey, String secretKey) {
        ...
        StringBuilder tradeStr = new StringBuilder("{'event':'addChannel','channel':'").append(channel)
                .append("','parameters':{'api_key':'").append(apiKey).append("','sign':'").append(signStr)
                .append("'},'binary':'true'}");
        Log.e(TAG, tradeStr.toString());
        this.sendMessage(tradeStr.toString());
    }
     ...
     * @param secretKey
     */
    public void subUserInfo(String apiKey, String secretKey) {
        Log.e(TAG, "apiKey=" + apiKey + ", secretKey=" + secretKey);
        StringBuilder preStr = new StringBuilder("api_key=");
        preStr.append(apiKey).append("&secret_key=").append(secretKey);
        String signStr = MD5Util.getMD5String(preStr.toString());
    ...
    public void realTrades(String apiKey, String secretKey) {
        ...
        StringBuilder tradeStr = new StringBuilder(
                "{'event':'addChannel','channel':'" + channel + "','parameters':{'api_key':'").append(apiKey)
                .append("','sign':'").append(signStr).append("'},'binary':'true'}");
        Log.e(TAG, tradeStr.toString());
        this.sendMessage(tradeStr.toString());
    }
...
class MoniterTask extends TimerTask {
    ...
    public void updateTime() {
        // Log.e(TAG,("startTime is update");
        startTime = System.currentTimeMillis();
    }

    public MoniterTask(WebSocketBase client) {
        this.client = client;
        // Log.e(TAG,("TimerTask is starting.... ");
    }

    public void run() {
        if (System.currentTimeMillis() - startTime > checkTime) {
            client.setStatus(false);
            // Log.e(TAG,("Moniter reconnect....... ");
            client.reConnect();
        }
        client.sentPing();
        // Log.e(TAG,("Moniter ping data sent.... ");
    }
